...
SCENARIOS = {
    "horizontal_cross": [
        {"object": "right_obj", "direction": "right", "start": (36, 20)},
        {"object": "left_obj", "direction": "left", "start": (36, 72)},
    ],
    "vertical_cross": [
        {"object": "down_obj", "direction": "down", "start": (10, 48)},
        {"object": "up_obj", "direction": "up", "start": (62, 48)},
    ],
    "diagonal_cross": [
        {"object": "down_right_obj", "direction": "down_right", "start": (14, 22)},
        {"object": "up_left_obj", "direction": "up_left", "start": (58, 74)},
    ],
}
...
def build_sequence(scenario_name, speed, box_size, noise_prob, num_frames, height, width, seed):
    rng = np.random.default_rng(seed)
    specs = SCENARIOS[scenario_name]
    spikes = np.zeros((num_frames, height, width), dtype=np.uint8)
    masks = {spec["object"]: np.zeros_like(spikes, dtype=bool) for spec in specs}
    overlap_mask = np.zeros_like(spikes, dtype=bool)
    margin = max(3, box_size + 1)

    for t in range(num_frames):
        occupied = np.zeros((height, width), dtype=bool)
        for spec in specs:
            dy, dx = DIRECTIONS[spec["direction"]]
            y0, x0 = spec["start"]
            ...
            obj_mask[y:y + box_size, x:x + box_size] = True
            overlap_mask[t] |= occupied & obj_mask
            masks[spec["object"]][t] = obj_mask
            occupied |= obj_mask
            spikes[t, obj_mask] = 1
        if noise_prob > 0:
            ...
            spikes[t, noise] = 1
    return spikes, masks, overlap_mask


def run_case(device, mapping, scenario_name, speed, box_size, noise_prob, num_frames, height, width, warmup, seed):
    estimator = motion_estimation(dvs_h=height, dvs_w=width, device=device, logger=DummyLogger())
    spikes, masks, overlap_mask = build_sequence(scenario_name, speed, box_size, noise_prob, num_frames, height, width, seed)
    specs = SCENARIOS[scenario_name]
    traces = {spec["object"]: [] for spec in specs}
    hits = {spec["object"]: [] for spec in specs}
    non_overlap_traces = {spec["object"]: [] for spec in specs}
    non_overlap_hits = {spec["object"]: [] for spec in specs}
    ...
    for t in range(num_frames):
        ...
        for spec in specs:
            ...
            obj_mask = masks[obj_name][t]
            values = motion_id_np[obj_mask]
            traces[obj_name].append(dominant_nonzero(values))
            hits[obj_name].append(float((values > 0).mean()))

            clean_mask = obj_mask & ~overlap_mask[t]
            if clean_mask.any():
                clean_values = motion_id_np[clean_mask]
                non_overlap_traces[obj_name].append((t, dominant_nonzero(clean_values)))
                non_overlap_hits[obj_name].append((t, float((clean_values > 0).mean())))
    ...
    for spec in specs:
        ...
        no_dirs = [item for frame_idx, item in non_overlap_traces[obj_name] if frame_idx >= warmup]
        no_hits = [item for frame_idx, item in non_overlap_hits[obj_name] if frame_idx >= warmup]
        rows.append(
            {
                ...
                "non_overlap_correct_rate": float(np.mean([item == expected_id for item in no_dirs])) if no_dirs else 0.0,
                "non_overlap_hit_rate": float(np.mean(no_hits)) if no_hits else 0.0,
            }
        )
    return rows
...
def plot_summary(output_dir: Path, aggregate_rows: list, speeds: list, box_sizes: list, noise_probs: list) -> None:
    for noise_prob in noise_probs:
        ...
        min_no_correct = np.zeros_like(min_correct)
        ...
        for row in subset:
            ...
            min_no_correct[bi, si] = row["min_non_overlap_correct_rate"]
            passed[bi, si] = 1.0 if row["passed"] else 0.0

        fig, axes = plt.subplots(1, 3, figsize=(14, 4))
        for ax, title, data in zip(
            axes,
            ["All-Frame Min Correct", "Non-Overlap Min Correct", "Pass Mask"],
            [min_correct, min_no_correct, passed],
        ):
            im = ax.imshow(data, cmap="viridis", vmin=0, vmax=1)
            ax.set_title(f"Overlap {title}, noise={noise_prob:g}")
            ax.set_xlabel("Speed")
        ...
        fig.tight_layout()
        fig.savefig(output_dir / f"multi_overlap_noise_{noise_prob:g}.png", dpi=200, bbox_inches="tight")
        plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(description="Multi-object overlapping trajectory motion direction sweep.")
    parser.add_argument("--output_dir", type=str, default="results/motion_multi_object_overlap_sweep")
                ...
                for scenario_index, scenario_name in enumerate(SCENARIOS):
                    rows.extend(
                        run_case(
                            ...
                            args.width,
                            args.warmup,
                            seed=7000 + scenario_index + int(noise_prob * 1_000_000) + box_size * 10 + speed,
                        )
                    )
                for row in rows:
                    row["passed"] = (
                        row["correct_rate"] >= args.correct_thr
                        and row["hit_rate"] >= args.hit_thr
                        and row["non_overlap_correct_rate"] >= args.correct_thr
                    )
                case_rows.extend(rows)
                failed = [f"{row['scenario']}:{row['object']}" for row in rows if not row["passed"]]
                aggregate_rows.append(
                    {
                        ...
                        "min_non_overlap_correct_rate": min(row["non_overlap_correct_rate"] for row in rows),
                        "mean_non_overlap_correct_rate": float(np.mean([row["non_overlap_correct_rate"] for row in rows])),
    ...
    write_csv(
        output_dir / "multi_overlap_cases.csv",
        case_rows,
        [
            "scenario",
            "object",
            ...
            "correct_rate",
            "hit_rate",
            "non_overlap_correct_rate",
            "non_overlap_hit_rate",
            "passed",
        ],
    )
    write_csv(
        output_dir / "multi_overlap_aggregate.csv",
        aggregate_rows,
        [
            "speed",
            "box_size",
            ...
            "min_correct_rate",
            "mean_correct_rate",
            "min_non_overlap_correct_rate",
            "mean_non_overlap_correct_rate",
            "min_hit_rate",
            "failed_objects",
            "passed",
        ],
